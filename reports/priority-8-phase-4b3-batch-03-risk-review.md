...
| Forbidden Dative + infinitive reconstruction | Clear | Both `infinitive` patterns (permission and enabling) have exactly one complement — the infinitive itself — verified by a dedicated test (`test_pozwalac_never_combines_dative_with_infinitive`). |
...
| CEFR/status decisions | Clear | 29 of 32 patterns are active-production with an independent justification per pattern-type; the three recognition-only patterns each have a documented, non-mechanical rationale (see the authoring report). |
| Provenance | Clear | 3 of 32 examples are truthfully marked `repository-reuse` and resolve byte-identically (verified independently, not only via the test file); the other 29 are `editorial-generated` and were checked for quiet collision against the repository index. |
...
- The 32 patterns keep alternative schemas separate: no seller/price/
...
- Every pattern has one, and only one, resolving example. All 32 Polish
  examples are unique, as are all 32 English translations.
- Pattern-key reuse across meanings of the same lemma (`pozwalać`'s
  `infinitive`; `wymagać`'s three shared keys; `kupować`/`kupić`'s three
...
2. Confirm the six-pattern, two-meaning `pozwalać` record and the
   six-pattern, two-meaning `wymagać` record remain pedagogically
   manageable rather than overloaded, given both reach the same pattern
   count as the largest Batch 1 records.
