...
**9. Exact clause distinctions** (`że`, `żeby`, interrogative-dependent, direct speech) — **`REQUIRES FUTURE MODEL/SCHEMA CHANGE`**. This is the most significant finding of this audit. `CLAUSE_KINDS = {"ze", "czy", "zeby", "interrogative"}` (`priority7_tooling.py`) — there is **no `direct-speech` value**. This audit searched `priority7_tooling.py`, `pp-verb-patterns.js`, every schema/spec JSON file, and every `reports/*.md` spec document: the string `direct-speech` (or `direct speech` as a schema token) appears **nowhere** in code or schema, only in Phase 3 linguistic-evidence prose. Yet Phase 3 evidence explicitly and repeatedly maps "direct speech" to the `clause` complement type as a real, verified alternative construction for multiple frozen-68 lemmas: `czytać`/`przeczytać`, `pisać`/`napisać`, `powiedzieć`, `zgadzać się`, `polecać`, `radzić`, `odpowiadać` (see `reports/priority-8-phase-3-batch-04.md` lines 127–189, `reports/priority-8-phase-3-batch-06-risk-review.md` lines 34–95). Phase 4 authoring cannot represent a direct-speech alternative as a `clause` complement until either (a) `CLAUSE_KINDS` gains a new enum value and the corresponding validators/runtime consumer are updated to accept it, or (b) a product decision is made to omit direct-speech alternatives from Phase 4 patterns entirely (which would silently under-represent evidence Phase 3 explicitly verified). **PHASE 4A-2 DECISION NEEDED.**

**10. Metadata-only aspect partner `zaczynać → zacząć`** — `SUPPORTED WITH EDITORIAL DISCIPLINE` (leaning toward `SUPPORTED AS-IS` for the narrow "no separate full-pattern record" requirement, but see caveat). The runtime schema already supports a lemma carrying `aspectPartnerIds` pointing at another lemma's ID without requiring reciprocal full pattern content to *duplicate* — `zacząć` (full record) can simply list `aspectPartnerIds: ["<zaczynać's lemma id>"]` or vice versa. The open question this audit surfaces but does not resolve: **does "metadata-only, no separate Phase 4 full-pattern record" mean `zaczynać` gets no lemma record in `editorial/verb-pattern-candidates.json` at all** (in which case `zacząć`'s `aspectPartnerIds` would point at an ID that resolves to nothing in the current corpus — the reciprocity validator in `priority7_tooling.py` checks the partner resolves *if present*, but this audit did not find a code path that tolerates a partner ID with zero corresponding lemma record anywhere in the same document), **or does it mean a minimal lemma-only stub record is created for `zaczynać` with no meanings** (which the schema's `meanings: []`-as-required-field shape may or may not permit — not tested, since doing so would allocate content and is out of scope for Phase 4A-1)? **PHASE 4A-2 DECISION NEEDED** — this is a concrete, answerable schema question, not a linguistic one, and it applies identically to `przeczytać → czytać`.

**11. Metadata-only aspect partner `przeczytać → czytać`** — same classification and same open question as item 10.
...
| 9 | Exact clause distinctions (incl. direct speech) | **REQUIRES FUTURE MODEL/SCHEMA CHANGE** |
| 10 | Metadata-only partner zaczynać→zacząć | SUPPORTED WITH EDITORIAL DISCIPLINE (open schema question) |
| 11 | Metadata-only partner przeczytać→czytać | SUPPORTED WITH EDITORIAL DISCIPLINE (open schema question) |
...
Only one item (**#9**) is classified `REQUIRES FUTURE MODEL/SCHEMA CHANGE` outright. Items #10/#11 surface a real open question that is schema-adjacent but may resolve either as "no change needed" (editorial-only) or "small schema clarification needed," depending on a Phase 4A-2 product decision this audit does not make.
