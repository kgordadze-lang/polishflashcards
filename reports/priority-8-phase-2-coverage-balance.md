# Priority 8 Phase 2 — construction and coverage balance

## Interpretation lock

All expansion figures below are **planning hypotheses** or **coverage opportunities** inherited from Phase 0. Only the current 30-lemma counts are released facts. No candidate frame has been linguistically verified in Phase 2, and a repository occurrence is not valency evidence.

| Dimension | Current released | Phase 0 addition range | Central planning final | Phase 2 treatment |
|---|---:|---:|---:|---|
| Direct Genitive | 7 | 4–13 | 15 | Preserve non-Accusative object opportunities in `próbować`, `życzyć`, `unikać`, `wymagać`, and `uczyć`; verify every sense. |
| Direct Dative | 9 | 9–23 | 24 | Strong recipient/experiencer queue: `dawać/dać`, `odpowiadać`, `polecać`, `pozwalać`, `radzić`, `życzyć`, `pasować`, `pokazywać`, `przynosić`. |
| Direct Accusative | 8 | 25–48 | 43 | Everyday growth is useful, but ordinary transitives are not allowed to displace harder constructions mechanically. |
...
- The selection usefully broadens infinitive, clause, preposition + Instrumental, and non-motion preposition + Genitive research while retaining everyday object verbs.
- Direct Dative is already a relative strength, but recipient/experiencer and multi-complement teaching remains high-value rather than redundant.
...
A replacement must first pass Phase 3 evidence and model fit. It should then repair the departing lemma's most important learner/domain contribution without worsening motion concentration, duplicating a current/selected lemma, or collapsing thin non-Accusative, clause, infinitive, lexical-`się`, or meaning-sensitive capacity. Raw matrix parity is never a selection goal.
