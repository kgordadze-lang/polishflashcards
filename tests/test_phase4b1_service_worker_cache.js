...
// =========================================================================
eq('A1 the app-shell cache is popolsku-v63', API.CACHE, 'popolsku-v63');
...
ok('A1 the source declares the shell cache exactly once',
   countOf(SW_CODE, 'const CACHE = "popolsku-v63"') === 1 &&
   ...
   countOf(SW_CODE, '"popolsku-v60"') === 0 && countOf(SW_CODE, '"popolsku-v61"') === 0 &&
   countOf(SW_CODE, '"popolsku-v62"') === 0);
...
[
  ...
  // popolsku-v62, the shell this release supersedes, and popolsku-v630, which merely
  ...
  ['popolsku-v62', true],
  ['popolsku-v590', true], ['popolsku-v630', true],
  ...
  ['popolsku-v63', false], ['popolsku-audio', false],
  ['popolsku-video', false], ['popolsku-vocabulary', false], ['popolsku-vectors', false],
  ['popolsku-v63-beta', false], ['popolsku-v', false], ['popolsku-v63.1', false],
...
(function () {
  var worker = makeWorker({ seed: {
    ...
    'popolsku-vectors': {}, 'popolsku-v63-beta': {}, 'popolsku-v': {}
  } });
  runActivate(worker);
  eq('A5 every obsolete shell cache is removed in one activation',
     ...
     ['popolsku-v53', 'popolsku-v54', 'popolsku-v55', 'popolsku-v58', 'popolsku-v59',
      'popolsku-v60', 'popolsku-v61', 'popolsku-v62']);
  eq('A5 the surviving cache names are exactly the protected ones',
     worker.caches.names.slice().sort(),
     ['popolsku-a2hs-backup', 'popolsku-audio', 'popolsku-v', 'popolsku-v63', 'popolsku-v63-beta',
...
(function () {
  ...
  eq('C4 activation keeps the new shell and the audio cache',
     worker.caches.names.slice().sort(), ['popolsku-audio', 'popolsku-v63']);
...
(function () {
  var worker = makeWorker({ seed: { 'popolsku-v63': { 'https://popolsku.app/': seeded('https://popolsku.app/', 'shell') } } });
...
(function () {
  var worker = makeWorker({
    seed: { 'popolsku-v63': { 'https://popolsku.app/': seeded('https://popolsku.app/', 'shell') } },
    route: function () { return { reject: new TypeError('offline') }; }
  });
...
[['data-a1.js', '/data-a1.js'], ['audio-manifest.json', '/audio-manifest.json']].forEach(function (row) {
  ...
  var offline = makeWorker({
    seed: { 'popolsku-v63': (function () { var s = {}; s[ORIGIN + row[1]] = seeded(ORIGIN + row[1], 'cached-' + row[0]); return s; })() },
    route: function () { return { reject: new TypeError('offline') }; }
  });
...
(function () {
  var worker = makeWorker({ seed: { 'popolsku-v63': {
...
// =========================================================================
var WRITE_BRANCHES = [
  { name: 'navigation', request: nav(ORIGIN + '/'), cache: 'popolsku-v63', key: ORIGIN + '/' },
  { name: 'generated page', request: nav(ORIGIN + '/guide/'), cache: 'popolsku-v63', key: ORIGIN + '/guide/' },
  { name: 'data file', request: get(ORIGIN + '/data-a1.js'), cache: 'popolsku-v63', key: ORIGIN + '/data-a1.js' },
  { name: 'audio manifest', request: get(ORIGIN + '/audio-manifest.json'), cache: 'popolsku-v63', key: ORIGIN + '/audio-manifest.json' },
  { name: 'mp3', request: get(ORIGIN + '/audio/ab12ef34.mp3'), cache: 'popolsku-audio', key: ORIGIN + '/audio/ab12ef34.mp3' },
  { name: 'static asset', request: get(ORIGIN + '/pp-answer.js'), cache: 'popolsku-v63', key: ORIGIN + '/pp-answer.js' },
  { name: 'font', request: get(ORIGIN + '/fonts/plus-jakarta-sans-v12-latin-regular.woff2'), cache: 'popolsku-v63', key: ORIGIN + '/fonts/plus-jakarta-sans-v12-latin-regular.woff2' },
  { name: 'web app manifest', request: get(ORIGIN + '/manifest.json'), cache: 'popolsku-v63', key: ORIGIN + '/manifest.json' }
];
...
(function () {
  ...
  var scheduled = worker.api.cacheWrite(event, 'popolsku-v63', ORIGIN + '/pp-answer.js', response);
  ...
  drain();
  eq('F9 the write landed', inv(worker, 'popolsku-v63'), [ORIGIN + '/pp-answer.js']);

  var event2 = new FetchEvent(get(ORIGIN + '/pp-answer.js'));
  eq('F9 a null key schedules nothing',
     worker.api.cacheWrite(event2, 'popolsku-v63', null, typed(ORIGIN + '/pp-answer.js')), false);
  eq('F9 an uncacheable response schedules nothing',
     worker.api.cacheWrite(event2, 'popolsku-v63', ORIGIN + '/x', typed(ORIGIN + '/x', { status: 404, ok: false })), false);
  ...
  var late = worker.api.cacheWrite(hostile, 'popolsku-v63', ORIGIN + '/pp-usage.js', typed(ORIGIN + '/pp-usage.js'));
  drain();
  eq('F9 a rejected waitUntil is contained', late, true);
  ok('F9 the write still completed',
     inv(worker, 'popolsku-v63').indexOf(ORIGIN + '/pp-usage.js') !== -1);
})();
...
(function () {
  ...
  var seed = { 'popolsku-v63': {}, 'popolsku-audio': {} };
  seed['popolsku-v63'][shared] = seeded(shared, 'shell-copy');
  seed['popolsku-v63'][shellSide] = seeded(shellSide, 'shell-script');
...
(function () {
  var clip = ORIGIN + '/audio/ab12ef34.mp3';
  var seed = { 'unrelated-tool-cache': {}, 'popolsku-v63': {}, 'popolsku-audio': {} };
  seed['unrelated-tool-cache'][clip] = seeded(clip, 'intruder');
  seed['popolsku-v63'][clip] = seeded(clip, 'shell-copy');
...
function shellSeed(key, response) { var o = {}; o[key] = response; return { 'popolsku-v63': o }; }
...
eq('G1 the app-shell cache is the new revision',
   (SW_SRC.match(/CACHE\s*=\s*"([^"]+)"/) || [])[1], 'popolsku-v63');
...
eq('G5 the worker is the only file that mentions the shell cache revision',
   [countOf(INDEX, 'popolsku-v63'), countOf(MANIFEST, 'popolsku-v63'), countOf(BUILD, 'popolsku-v63')],
   [0, 0, 0]);
