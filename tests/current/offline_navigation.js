...
eq('A1 shell cache revision is __CURRENT_SHELL_CACHE__',SHELL,'__CURRENT_SHELL_CACHE__');
...
eq('D1 APP_VERSION is the __CURRENT_APP_VERSION__ release',(INDEX.match(/APP_VERSION\s*=\s*"([^"]+)"/)||[])[1],'__CURRENT_APP_VERSION__');
eq('D1 shell cache matches the current release marker',(SW_SRC.match(/const CACHE\s*=\s*"([^"]+)"/)||[])[1],'__CURRENT_SHELL_CACHE__');
