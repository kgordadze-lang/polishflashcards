...
// =========================================================================
eq('R0 the shell cache is the current shell revision', SHELL, '__CURRENT_SHELL_CACHE__');
...
(function () {
  ...
  eq('S1 the shipped library matches the current manifest', clips, __CURRENT_AUDIO_COUNT__);
...
eq('S23 audio writes retain an event-bound lifetime helper',
   [countOf(SW_CODE, 'keepAlive(event, write);'), countOf(SW_CODE, 'function keepAlive(')],
   [1, 1]);
...
ok('S22 the audio diagnostics are all counters',
   Object.keys(W0.self.ppSwState.audio).every(function (key) {
     return typeof W0.self.ppSwState.audio[key] === 'number';
   }));
...
// =========================================================================
ok('W1 the app shell retains every learner-facing audio string',
   [FALLBACK_MSG, FAILED_MSG, RETRY_MSG].every(function (message) {
     return INDEX_CODE.indexOf(message) !== -1;
   }));
(function () {
...
eq('W5 APP_VERSION is the __CURRENT_APP_VERSION__ release', (INDEX.match(/APP_VERSION\s*=\s*"([^"]+)"/) || [])[1], '__CURRENT_APP_VERSION__');
eq('W5 the shell cache is the current shell revision',
   (SW_SRC.match(/const CACHE\s*=\s*"([^"]+)"/) || [])[1], '__CURRENT_SHELL_CACHE__');
...
ok('W6 category-aware media validation still gates reads and writes',
   countOf(SW_CODE, 'function isCacheableResponse(') === 1 &&
   countOf(SW_CODE, 'function hasExpectedMediaType(') === 1 &&
   countOf(SW_CODE, 'return hasExpectedMediaType(res, key);') === 1 &&
...
(function () {
  ...
  eq('W8 the audio manifest identity is unchanged',
     [ids.length, Object.keys(files).length], [__CURRENT_AUDIO_COUNT__, __CURRENT_AUDIO_COUNT__]);
})();
