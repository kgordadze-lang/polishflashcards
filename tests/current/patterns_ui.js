...
// The historical wrapper fixture predates the maintained public format. The
// current suite upgrades only its public envelope marker before exercising the
// shipping validator and derived views.
WRAPPED_FIXTURE.runtimeProjection.formatVersion = 2;
var RELEASE_REHEARSAL_FIXTURE = JSON.parse(
  readFile(ROOT + 'tests/fixtures/priority7/release-runtime-fixture.json'));
RELEASE_REHEARSAL_FIXTURE.formatVersion = 2;
...
['outerHTML', 'insertAdjacentHTML', 'document.write', 'eval('].forEach(function (token) {
  eq('A3 the reference renderers never use ' + token, countOf(PATTERNS_CODE, token), 0);
});
eq('A3 innerHTML is used only for the trusted shared audio SVG',
   [countOf(PATTERNS_CODE, 'innerHTML'), countOf(PATTERNS_CODE, 'audio.innerHTML = G_AUDIO;')],
   [1, 1]);
...
ok('A5 the screen stays out of the hash deep-link allowlist',
   /\[[^\]]*"privacy"[^\]]*\]\.includes\(ppInitialScreen\)/.test(INDEX) &&
   !/\[[^\]]*"patterns"[^\]]*\]\.includes\(ppInitialScreen\)/.test(INDEX));
...
eq('C2 a wrong format version is rejected',
   [acceptClone(function (d) { d.formatVersion = 1; }),
    acceptClone(function (d) { d.formatVersion = '2'; })], [false, false]);
...
eq('D5 every closed role has exactly one fixed English phrasing',
   ...
   ['content', 'experiencer', 'interlocutor', 'means', 'object', 'predicate',
    'recipient', 'source', 'subject', 'target', 'topic']);
...
eq('L2 the app version is the __CURRENT_APP_VERSION__ release',
...
eq('M9 the version and cache markers moved together in the I1 release',
   [(INDEX.match(/APP_VERSION\s*=\s*"([^"]+)"/) || [])[1],
    ...
    countOf(readFile(ROOT + 'sw.js'), '__CURRENT_SHELL_CACHE__'),
    countOf(readFile(ROOT + 'sw.js'), 'verb-patterns')], ['__CURRENT_APP_VERSION__', 0, 1, 3]);
