...
// =========================================================================
eq('A1 the app-shell cache is __CURRENT_SHELL_CACHE__', API.CACHE, '__CURRENT_SHELL_CACHE__');
...
ok('A1 the source declares the shell cache exactly once',
   countOf(SW_CODE, 'const CACHE = "__CURRENT_SHELL_CACHE__"') === 1 &&
   ...
   countOf(SW_CODE, '"popolsku-v63"') === 0 && countOf(SW_CODE, '"popolsku-v64"') === 0);
...
[
  ...
  // popolsku-v65, the shell this release supersedes, and __CURRENT_SHELL_CACHE__0, which merely
  ...
  ['popolsku-v65', true],
  ['popolsku-v590', true], ['popolsku-v650', true], ['__CURRENT_SHELL_CACHE__0', true],
  ...
  ['__CURRENT_SHELL_CACHE__', false], ['popolsku-audio', false],
  ['popolsku-video', false], ['popolsku-vocabulary', false], ['popolsku-vectors', false],
  ['__CURRENT_SHELL_CACHE__-beta', false], ['popolsku-v', false], ['__CURRENT_SHELL_CACHE__.1', false],
...
(function () {
  var worker = makeWorker({ seed: {
    ...
    'popolsku-v63': {}, 'popolsku-v64': {}, 'popolsku-v65': {}, '__CURRENT_SHELL_CACHE__': {},
    'popolsku-audio': {}, 'popolsku-a2hs-backup': {},
    'some-other-cache': {}, 'popolsku-video': {}, 'popolsku-vocabulary': {},
    'popolsku-vectors': {}, '__CURRENT_SHELL_CACHE__-beta': {}, 'popolsku-v': {}
  } });
  runActivate(worker);
  ...
  eq('A5 the surviving cache names are exactly the protected ones',
     ...
     ['popolsku-a2hs-backup', 'popolsku-audio', 'popolsku-v', '__CURRENT_SHELL_CACHE__', '__CURRENT_SHELL_CACHE__-beta',
...
ok('C1 index.html registers the worker without a skip-waiting handshake',
   INDEX.indexOf('navigator.serviceWorker.register("sw.js")') !== -1 &&
   INDEX.indexOf('skipWaiting') === -1);
...
(function () {
  ...
  eq('C4 activation keeps the new shell and the audio cache',
     worker.caches.names.slice().sort(), ['popolsku-audio', '__CURRENT_SHELL_CACHE__']);
...
(function () {
  var worker = makeWorker({ seed: { '__CURRENT_SHELL_CACHE__': { 'https://popolsku.app/': seeded('https://popolsku.app/', 'shell') } } });
...
(function () {
  var worker = makeWorker({
    seed: { '__CURRENT_SHELL_CACHE__': { 'https://popolsku.app/': seeded('https://popolsku.app/', 'shell') } },
    route: function () { return { reject: new TypeError('offline') }; }
  });
...
[['data-a1.js', '/data-a1.js'], ['audio-manifest.json', '/audio-manifest.json']].forEach(function (row) {
  ...
  var offline = makeWorker({
    seed: { '__CURRENT_SHELL_CACHE__': (function () { var s = {}; s[ORIGIN + row[1]] = seeded(ORIGIN + row[1], 'cached-' + row[0]); return s; })() },
    route: function () { return { reject: new TypeError('offline') }; }
  });
...
(function () {
  var worker = makeWorker({ seed: { '__CURRENT_SHELL_CACHE__': {
...
// =========================================================================
var WRITE_BRANCHES = [
  { name: 'navigation', request: nav(ORIGIN + '/'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/' },
  { name: 'generated page', request: nav(ORIGIN + '/guide/'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/guide/' },
  { name: 'data file', request: get(ORIGIN + '/data-a1.js'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/data-a1.js' },
  { name: 'audio manifest', request: get(ORIGIN + '/audio-manifest.json'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/audio-manifest.json' },
  { name: 'mp3', request: get(ORIGIN + '/audio/ab12ef34.mp3'), cache: 'popolsku-audio', key: ORIGIN + '/audio/ab12ef34.mp3' },
  { name: 'static asset', request: get(ORIGIN + '/pp-answer.js'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/pp-answer.js' },
  { name: 'font', request: get(ORIGIN + '/fonts/plus-jakarta-sans-v12-latin-regular.woff2'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/fonts/plus-jakarta-sans-v12-latin-regular.woff2' },
  { name: 'web app manifest', request: get(ORIGIN + '/manifest.json'), cache: '__CURRENT_SHELL_CACHE__', key: ORIGIN + '/manifest.json' }
];
...
(function () {
  ...
  eq('F8 writes landed in exactly the two application caches',
     worker.caches.names.slice().sort(), ['popolsku-audio', '__CURRENT_SHELL_CACHE__']);
  ok('F8 the source has no fire-and-forget cache write',
     SW_CODE.indexOf('.then(c => c.put(') === -1);
...
(function () {
  ...
  var scheduled = worker.api.cacheWrite(event, '__CURRENT_SHELL_CACHE__', ORIGIN + '/pp-answer.js', response);
  ...
  drain();
  eq('F9 the write landed', inv(worker, '__CURRENT_SHELL_CACHE__'), [ORIGIN + '/pp-answer.js']);

  var event2 = new FetchEvent(get(ORIGIN + '/pp-answer.js'));
  eq('F9 a null key schedules nothing',
     worker.api.cacheWrite(event2, '__CURRENT_SHELL_CACHE__', null, typed(ORIGIN + '/pp-answer.js')), false);
  eq('F9 an uncacheable response schedules nothing',
     worker.api.cacheWrite(event2, '__CURRENT_SHELL_CACHE__', ORIGIN + '/x', typed(ORIGIN + '/x', { status: 404, ok: false })), false);
  ...
  var late = worker.api.cacheWrite(hostile, '__CURRENT_SHELL_CACHE__', ORIGIN + '/pp-usage.js', typed(ORIGIN + '/pp-usage.js'));
  drain();
  eq('F9 a rejected waitUntil is contained', late, true);
  ok('F9 the write still completed',
     inv(worker, '__CURRENT_SHELL_CACHE__').indexOf(ORIGIN + '/pp-usage.js') !== -1);
})();
...
// Every application read continues to go through the named helper; additional
// current product surfaces may add call sites without weakening isolation.
ok('I1 every application read goes through it', countOf(SW_CODE, 'cacheMatch(') >= 4);
...
[
  { name: 'root shell key', seedKey: ORIGIN + '/', request: nav(ORIGIN + '/'),
    ownCache: '__CURRENT_SHELL_CACHE__' },
...
(function () {
  ...
  var seed = { '__CURRENT_SHELL_CACHE__': {}, 'popolsku-audio': {} };
  seed['__CURRENT_SHELL_CACHE__'][shared] = seeded(shared, 'shell-copy');
  seed['__CURRENT_SHELL_CACHE__'][shellSide] = seeded(shellSide, 'shell-script');
...
(function () {
  var clip = ORIGIN + '/audio/ab12ef34.mp3';
  var seed = { 'unrelated-tool-cache': {}, '__CURRENT_SHELL_CACHE__': {}, 'popolsku-audio': {} };
  seed['unrelated-tool-cache'][clip] = seeded(clip, 'intruder');
  seed['__CURRENT_SHELL_CACHE__'][clip] = seeded(clip, 'shell-copy');
...
function shellSeed(key, response) { var o = {}; o[key] = response; return { '__CURRENT_SHELL_CACHE__': o }; }
...
ok('J13 the read gate is the same function as the write gate',
...
/* Retention still deletes entries rather than clearing a whole cache. Current
   offline-audio removal intentionally owns a separate whole-cache operation. */
ok('J13 retention does not clear the whole audio cache',
   SW_CODE.indexOf('function trimAudioCache(') !== -1 &&
   SW_CODE.indexOf('cache.delete(entry.request)') !== -1);
(function () {
...
eq('G1 the app-shell cache is the new revision',
   (SW_SRC.match(/CACHE\s*=\s*"([^"]+)"/) || [])[1], '__CURRENT_SHELL_CACHE__');
...
ok('G3 the audio contract is unchanged',
   /fetch\("audio-manifest\.json",\s*\{\s*cache:\s*"no-store"\s*\}\)/.test(INDEX) &&
...
eq('G5 the worker is the only file that mentions the shell cache revision',
   [countOf(INDEX, '__CURRENT_SHELL_CACHE__'), countOf(MANIFEST, '__CURRENT_SHELL_CACHE__'), countOf(BUILD, '__CURRENT_SHELL_CACHE__')],
   [0, 0, 0]);
