"""Priority 8 Phase 4D1 regression coverage for Verb Patterns audio-icon parity."""
...
def extract_function(source: str, name: str) -> str:
    """Return one ordinary function body without matching unrelated code."""
...
class Priority8Phase4D1SpeakerIconUiTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.renderer = extract_function(INDEX, "pRenderLemma")

    def test_verb_patterns_reuses_the_established_shared_svg(self):
        self.assertIn(
    ...
    def test_existing_pattern_audio_wiring_and_readiness_are_preserved(self):
    ...
    def test_shared_icon_styling_and_state_colours_remain_applicable(self):
        self.assertIn(
            ".mini-audio{flex:0 0 auto;width:36px;height:36px;border-radius:50%;"
            "border:none;background:#fff;color:var(--mint-text);",
            INDEX,
        )
    ...
    def test_existing_audio_player_and_fallback_paths_are_unchanged(self):
    ...
    def test_release_and_audio_inputs_remain_immutable(self):
